...
def actionList(s, l, st, t):
    print t
    t.level = len(t[0].getText())
    #t.contentNode = t.findFlatByName("listItemContent")
    # if t.contentNode is None:
    #    raise ParseException(s, l, "a list item needs content")

#listItemEnd = buildRegex(ur"\n")

#unorderedList = buildRegex(ur"^- ") + content + listItemEnd
#unorderedList= unorderedList.setResultsNameNoCopy("unorderedList").setParseAction(actionList)
...
def actionImgLink(s, l, st, t):
    t.appendixNode = t.findFlatByName("urlModeAppendix")

    t.url = "rel://files/" + t.findFlatByName("url").getString()
    t.titleNode = t.findFlatByName("title")
#     print "--actionUrlLink3", repr(t.url)
...
wikiWordNcc = bracketStart + bracketStart + \
        ...
        Optional(title) + bracketEnd + bracketEnd + \
...
urlTitled = bracketStart + urlWithAppend + \
        Optional(title) + bracketEnd
urlTitled = urlTitled.setResultsNameNoCopy("urlLink").setParseAction(actionUrlLink)

imgFileName = buildRegex("[A-Za-z0-9\-\_\.]+", "url")
imgLink = bracketStart + bracketStart + buildRegex(ur"Image") + buildRegex(ur"[ \t]*[=:]") + imgFileName + bracketEnd + bracketEnd
imgLink = imgLink.setResultsNameNoCopy("urlLink").setParseAction(actionImgLink)

urlRef = imgLink | urlTitled | urlBare
...
#propInsKey = buildRegex(ur"Category", "key")

property = bracketStart + bracketStart + Optional(ur":") + propInsKey + \
        buildRegex(ur"[ \t]*[=:]") + propInsValue + \
        Optional(buildRegex(ur"\|.*")) + bracketEnd + bracketEnd
property = property.setResultsNameNoCopy("property").setParseAction(actionProperty)

# [CD] removed insertions
...
findMarkupInCell = FindFirst([bold, italics, noExportSingleLine,
        ...
        urlRef, escapedChar, footnote, wikiWord,
...
findMarkupInTitle = FindFirst([bold, italics, noExportSingleLine,
        suppressHighlightingSingleLine,
        urlRef, escapedChar, footnote, htmlTag, htmlEntity],
        endTokenInTitle)
...
findMarkupInHeading = FindFirst([bold, italics, noExportSingleLine,
        suppressHighlightingSingleLine,
        urlRef, escapedChar, footnote, wikiWord, htmlTag,
        htmlEntity], endToken)
...
findMarkupInTodo = FindFirst([bold, italics, noExportSingleLine,
        suppressHighlightingSingleLine,
        urlRef, property, escapedChar, footnote, wikiWord,   # wikiWordNcc, wikiWordCc,
...
findMarkup = FindFirst([bold, italics, noExportSingleLine,
        ...
        property, escapedChar, footnote, wikiWord,
...
def describeWikiLanguage(ver, app):
    ...
    return (("wikidpad_mediawiki_1_0", u"WikidPad MediaWiki 1.0", parserFactory,
